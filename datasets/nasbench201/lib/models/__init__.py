...
def get_cell_based_tiny_net(config):
    ...
    if super_type == 'basic' and config.name in group_names:
        ...
        return TinyNetwork(config.C, config.N, genotype, config.num_classes, config.C, config.depth)
...
#         raise NotImplementedError
        from .cell_infers import NetworkCIFAR
        return NetworkCIFAR( config.C, config.num_classes, config.layers, False, config.genotype, config.depth, config.use_stem)
    else:
